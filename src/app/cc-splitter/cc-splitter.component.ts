import { Component, Input, Output, ChangeDetectionStrategy, ChangeDetectorRef, Renderer2, AfterViewInit, OnDestroy, ElementRef, NgZone, ViewChildren, QueryList, EventEmitter, TemplateRef } from '@angular/core';
import { Observable, Subscriber, Subject } from 'rxjs';
import { debounceTime } from 'rxjs/operators';

import { IArea, IPoint, ISplitSnapshot, IAreaSnapshot, IOutputData, IOutputAreaSizes } from './iterface';
import { CcSplitterAreaDirective } from './cc-splitter-area.directive';
...
@Component({
  selector: 'cc-splitter',
  exportAs: 'ccSplitter',
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './cc-splitter.component.html',
  styleUrls: ['./cc-splitter.component.scss']
})
export class CcSplitterComponent implements AfterViewInit, OnDestroy {
  ...
  @Output() get transitionEnd(): Observable<IOutputAreaSizes> {
      return new Observable<IOutputAreaSizes>(subscriber => this.transitionEndSubscriber = subscriber).pipe(
  ...
  @Input('defaultGutter') defaultGutterRef: TemplateRef<HTMLElement>;
  ...
  public addArea(component: CcSplitterAreaDirective): void {
      const newArea: IArea = {
          component, 
          order: 0, 
          size: 0,
  ...
  public removeArea(component: CcSplitterAreaDirective): void {
  ...
  public updateArea(component: CcSplitterAreaDirective, resetOrders: boolean, resetSizes: boolean): void {
      if(component.visible === true) {
          this.build(resetOrders, resetSizes);
      }
  }

  public showArea(component: CcSplitterAreaDirective): void {
      const area = this.hidedAreas.find(a => a.component === component);
      if(area === undefined) {
          return;
      }
  ...
  public hideArea(comp: CcSplitterAreaDirective): void {
